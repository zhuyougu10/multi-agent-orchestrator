...
```
从 GitHub 下载 multi-agent-orchestrator 的组件到当前项目：
1. 下载 setup.ps1 脚本
2. 运行 .\setup.ps1 -RepoUrl "https://github.com/你的用户名/multi-agent-orchestrator" 安装所需组件
...
如果你想把这个框架安装到已有的项目中，只需下载特定组件：

```powershell
# 1. 下载安装脚本
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/你的用户名/multi-agent-orchestrator/main/setup.ps1" -OutFile "setup.ps1"

# 2. 运行安装脚本（从 GitHub 下载组件，不会污染项目目录）
.\setup.ps1 -RepoUrl "https://github.com/你的用户名/multi-agent-orchestrator"

# 3. 选择性安装（可选）
.\setup.ps1 -SkipGlobalSkills  # 跳过全局技能
.\setup.ps1 -SkipMcp           # 跳过 MCP 服务器
.\setup.ps1 -SkipTemplates     # 跳过模板
.\setup.ps1 -SkipAgents        # 跳过代理配置
.\setup.ps1 -SkipCommands      # 跳过命令
```
...
```powershell
# 克隆项目
git clone https://github.com/你的用户名/multi-agent-orchestrator.git
...
| 参数 | 说明 |
|------|------|
| `-RepoUrl` | GitHub 仓库地址（默认：本仓库） |
| `-Branch` | 分支名称（默认：main） |
| `-SkipGlobalSkills` | 跳过 superpowers 和 planning-with-files |
| `-SkipMcp` | 跳过 MCP task-router |
| `-SkipTemplates` | 跳过模板文件 |
| `-SkipAgents` | 跳过代理配置 |
| `-SkipCommands` | 跳过命令文件 |
| `-Force` | 强制重新下载 |
