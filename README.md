...
```
下载 https://raw.githubusercontent.com/你的用户名/multi-agent-orchestrator/main/setup.ps1 到临时目录，然后运行它安装多智能体编排框架的所有依赖
```
...
2. 下载安装脚本
   从 https://raw.githubusercontent.com/你的用户名/multi-agent-orchestrator/main/setup.ps1 下载脚本到当前目录

3. 运行安装脚本
   执行: ./setup.ps1 -GitHubRepo "你的用户名/multi-agent-orchestrator"
   这将安装：
...
```powershell
# 完整安装（推荐）
.\setup.ps1 -GitHubRepo "你的用户名/multi-agent-orchestrator"

# 只安装技能，不下载项目组件
.\setup.ps1

# 只安装项目配置，跳过全局技能（适合已有 superpowers 的用户）
.\setup.ps1 -GitHubRepo "你的用户名/multi-agent-orchestrator" -SkipGlobalSkills

# 强制覆盖已存在的文件
.\setup.ps1 -GitHubRepo "你的用户名/multi-agent-orchestrator" -Force
```
